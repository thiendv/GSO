...
/**
 * 購入データベースと UI をアップデートするためのメソッドを含んだ Handler<br>
 * 
...
public class ResponseHandler {
    ...
    /**
     * 販売用のアイテムを購入するユーザーのための新しいアクティビティを開始<br>
        ...
        new Thread(new Runnable() {
			...
			public void run() {
            	if(purchaseState == PurchaseState.PURCHASED){
            		// 購入時はDBに挿入
            		if(mPurchaseDao == null) mPurchaseDao = new PurchaseDao(context);
                	mPurchaseDao.insert(orderId, productId, developerPayload);
            	}
