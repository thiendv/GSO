...
public class Security {
     ...
     */
    public static ArrayList<VerifiedPurchase> verifyPurchase(Context con, String signedData, String signature) throws NoSuchAlgorithmException, InvalidKeySpecException, Base64DecoderException {
        if (signedData == null) {
            Logput.e("data is null");
            return null;
        }
        ...
        if (!TextUtils.isEmpty(signature)) {
             ...
             */
            String base64EncodedPublicKey = con.getString(R.string.inAppBilling_publicKey);
            PublicKey key = Security.generatePublicKey(base64EncodedPublicKey);
            verified = Security.verify(key, signedData, signature);
            if (!verified) {
                Logput.w("signature does not match data.");
                return null;
            }
        }
        ...
        if (!Security.isNonceKnown(nonce)) {
            Logput.w("Nonce not found: " + nonce);
            return null;
        }
    ...
    /**
     * Base64でエンコードされた公開鍵を含む文字列からPublicKeyインスタンスを生成します<br>
     * 
     * Generates a PublicKey instance from a string containing the
     * Base64-encoded public key.
     *
     * @param encodedPublicKey Base64-encoded public key (Base64でエンコードされた公開鍵)
     * @throws IllegalArgumentException if encodedPublicKey is invalid
     */
    public static PublicKey generatePublicKey(String encodedPublicKey) throws NoSuchAlgorithmException, InvalidKeySpecException, Base64DecoderException{
        try {
        	...
        	if(cause == null){
        		...
        		message = "ERROR : " + e.getMessage() + ":" + cause.toString();
        	}
        	Logput.e(message, e);
    		throw new RuntimeException(message, e);
        } catch (InvalidKeySpecException e) {
            Logput.e("ERROR : Invalid key specification.");
            throw new IllegalArgumentException("Invalid key specification.", e);
        } catch (Base64DecoderException e) {
            Logput.e("ERROR : Base64 decoding failed.");
    ...
    public static boolean verify(PublicKey publicKey, String signedData, String signature) {
        ...
        try {
            ...
            if (!sig.verify(Base64.decode(signature))) {
                Logput.e("Signature verification failed.");
                return false;
            }
            return true;
        } catch (NoSuchAlgorithmException e) {
            Logput.e("NoSuchAlgorithmException.");
        } catch (InvalidKeyException e) {
            Logput.e("Invalid key specification.");
        } catch (SignatureException e) {
            Logput.e("Signature exception.");
        } catch (Base64DecoderException e) {
            Logput.e("Base64 decoding failed.");
        }
        return false;
    }
}
